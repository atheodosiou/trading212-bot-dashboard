...
@Component({
  selector: 'app-t212-dividends',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [],
  templateUrl: './t212-dividends.html',
})
export class T212DividendsPage implements OnInit {
  ...
  readonly dividends = signal<T212Dividend[]>([]);
  ...
  load(): void {
    ...
    this.error.set('');
    this.api.getDividends({ year: this.selectedYear(), limit: 200 }).subscribe({
      next: result => {
        this.dividends.set(result.items);
        this.total.set(result.total);
        this.loading.set(false);
      },
      error: (err: HttpErrorResponse) => {
        this.error.set(this.extractMessage(err, 'Failed to load dividends.'));
        this.loading.set(false);
      },
    });
  }
